...
public class MainApplicationSelectedTabChangeListener implements TabSheet.SelectedTabChangeListener {
    ...
    public void selectedTabChange(SelectedTabChangeEvent event) {
        if (source instanceof BreedingManagerMainView) {
            
            try {
                ((BreedingManagerMainView) source).tabSheetSelectedTabChangeAction(event.getTabSheet());
            } catch (InternationalizableException e) {
                LOG.error(e.toString() + "\n" + e.getStackTrace());
                e.printStackTrace();
                MessageNotifier.showError(e.getCaption(), e.getDescription()); 
            }
