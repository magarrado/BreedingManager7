...
public class CrossingManagerDetailsComponent extends AbsoluteLayout 
    ...
    public void doneButtonClickAction() throws InternationalizableException{
        if (validateRequiredFields()){
            ...
            ConfirmDialog.show(UI.getCurrent(), messageSource.getMessage(Message.SAVE_CROSSES_MADE), 
    ...
    private void saveRecords() {
        ...
        try {
            ...
            MessageNotifier.showMessage(messageSource.getMessage(Message.SUCCESS), 
                    messageSource.getMessage(Message.CROSSES_SAVED_SUCCESSFULLY), 3000, Position.MIDDLE_CENTER);
            ...
            e.printStackTrace();
            MessageNotifier.showError(messageSource.getMessage(Message.ERROR_DATABASE), 
                messageSource.getMessage(Message.ERROR_IN_SAVING_CROSSES_DEFINED), Position.MIDDLE_CENTER);
        }
    ...
    private boolean validateRequiredFields(){
        return 
        CrossingManagerUtil.validateRequiredStringField(germplasmListName, 
            messageSource, (String) germplasmListNameLabel.getCaption())
        
        && CrossingManagerUtil.validateRequiredStringField(germplasmListDescription, 
            messageSource,     (String) germplasmListDescriptionLabel.getCaption())
            
        && CrossingManagerUtil.validateRequiredField(germplasmListType, 
            messageSource, (String) germplasmListTypeLabel.getCaption())
            
        && CrossingManagerUtil.validateRequiredField(germplasmListDate, 
