...
public class AdditionalDetailsCrossNameComponent extends AbsoluteLayout 
        ...
        if (validateCrossNameFields()) {
            ...
            try {
                ...
                MessageNotifier.showError(messageSource.getMessage(Message.ERROR_DATABASE),
                        messageSource.getMessage(Message.ERROR_IN_GETTING_NEXT_NUMBER_IN_CROSS_NAME_SEQUENCE), Position.MIDDLE_CENTER);
            }
        }
    ...
    private boolean validateCrossNameFields(){
        ...
        if (StringUtils.isEmpty(prefix)){
            MessageNotifier.showError("Error with Cross Code", messageSource.getMessage(Message.ERROR_ENTER_PREFIX_FIRST), Position.MIDDLE_CENTER);
            return false;
        
        } else if (prefix.contains(" ")){
            MessageNotifier.showError("Error with Cross Code", messageSource.getMessage(Message.ERROR_PREFIX_HAS_WHITESPACE), Position.MIDDLE_CENTER);
            return false;
        } 
    ...
    private boolean validateGeneratedName(){
        ...
        if (nextNumberInSequence == null ){
            MessageNotifier.showError("Error with Cross Code", MessageFormat.format(
